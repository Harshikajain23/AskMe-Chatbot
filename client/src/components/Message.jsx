...
const Message = ({message}) => {
  ...
  return (
    <div>
      {message.role === "user" ? (
        <div className='flex items-start justify-end my-4 gap-2'>
          ...
          <span className='text-xs text-gray-400 dark:text-[#b8b2c0]'>{moment(message.timestamp).fromNow()}</span>
           </div> 
    ...
    : 
    (
      <div className='inline-flex flex-col gap-2 p-2 px-4 max-w-2xl bg-primary/20 dark:bg-[#464444] border border-[#827e87]/30 rounded-md my-4'> 
      {message.isImage ? (
